...
#include "control/jobs/AnkiExportJob.h"
    ...
    if (fileVersion > FILE_FORMAT_VERSION) {
        ...
        XojMsgBox::askQuestion(
                ...
                _("The file being loaded has a file format version newer than the one currently supported by this "
                  "version of AnkiXourj, so it may not load properly. Open anyways?"),
...
void Control::exportAsAnki() {
    this->clearSelectionEndText();
    auto* job = new AnkiExportJob(this);
    job->showDialogAndRun();
}
